...
public class Chp13_Graph{
	...
	/**
	 * same as dfs(). just print the path each time you visit a node
	 * @param index
	 */
	public void mst(int index){
		...
		Vertex vertex = vertexList[index];
		vertex.visited = true;
		//vertex.displayVertex();     this was printed in dfs()
		vertexStack.push(vertex);
		...
		while(!vertexStack.isEmpty()){
			...
			if(unvisitedAdjacentVertex != null){
				unvisitedAdjacentVertex.visited = true;
				//unvisitedAdjacentVertex.displayVertex();    this was printed in dfs()
				System.out.println(stackTop.label + "-" + unvisitedAdjacentVertex.label);
				vertexStack.push(unvisitedAdjacentVertex);
			}else{
				vertexStack.pop();
			}
